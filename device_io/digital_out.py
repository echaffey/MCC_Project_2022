...
def set_digital_out(board_num: int=0, bit_value: int= 0) -> None:
    ...
    try:
        ...
        print(f'Error in trying to connect to the device on board number {board_num}/n/n'
        'Error: /n{e}')
    finally:
        pass
    ...
    if not port: raise Exception(f'The device connected to board {board_num} does not support digital input')
